...
**Docker 容器网络修复**：Docker bridge 容器（172.16.0.0/12）的出站流量经宿主机 NAT 后走旁路由 TPROXY 时连接异常。通过策略路由让 Docker 网段流量直接走主路由 192.168.50.1 出去，绕过 TPROXY。

已通过 systemd 服务 `x5-docker-network.service` 持久化：
```bash
systemctl status x5-docker-network   # 查看状态
# 原理：ip rule from 172.16.0.0/12 → table docker-direct (default via 192.168.50.1)
```

> 注：此方案让 Docker 容器走主路由直连，不经过旁路由翻墙。如果容器需要翻墙，
> 在容器内配置 HTTP 代理环境变量指向旁路由：`http_proxy=http://192.168.50.2:10800`

**网关容灾**：当旁路由不可用（网关回退到 192.168.50.1）时，NPM 反向代理功能不受影响，仅 X5_Server 自身失去翻墙能力。Docker 容器不受旁路由故障影响（已走主路由直出）。
